...
Page({
  ...
  authorityJudge: function () {
    ...
    if (type == 1 && !this.data.cusmPrivilege) {
      ...
      wx.navigateTo({
        url: '../../original/openOrder/unitSelect/unitSelect?type=' + type
      })
    }
  },
  //买单
  order: function () {
    if (this.data.shopCart.length > 0) {
      wx.navigateTo({
        url: '../../original/openOrder/orderSure/orderSure?type=' + this.data.type + "&totalNum=" + this.data.totalData.totalNum
      })
    } else {
      wx.showToast({
        title: '请先选择货品',
